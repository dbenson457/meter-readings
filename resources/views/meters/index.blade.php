...
<div class="bg-white rounded-lg shadow-md overflow-hidden">
    <div class="p-6 border-b">
        ...
        <form id="searchForm" action="{{ route('meters.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div>
                <input type="text" name="search" id="searchInput" placeholder="Search by MPXN" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" value="{{ request('search') }}">
            </div>
        </form>
    </div>
            ...
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                        Type
                        <select name="type" id="typeFilterHeader" class="shadow appearance-none border rounded py-1 px-2 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
            ...
            <tbody id="meterTableBody" class="bg-white divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <a href="{{ route('meters.show', $meter) }}" class="text-blue-600 hover:text-blue-900 bg-blue-50 px-3 py-1 rounded-full">View Details</a>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <form action="{{ route('meters.destroy', $meter) }}" method="POST" class="inline-block" onsubmit="return confirm('Are you sure you want to delete this meter?');">
                            @csrf
                            @method('DELETE')
