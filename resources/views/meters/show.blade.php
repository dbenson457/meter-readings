        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <p class="mt-1">
                        <span class="px-3 py-1 inline-flex text-sm font-semibold rounded-full {{ $meter->type === 'electricity' ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800' }}">
                            {{ ucfirst($meter->type) }}
                        </span>
                    </p>
                </div>
                <div>
                    ...
                    <p class="text-lg font-semibold text-gray-800">{{ \Carbon\Carbon::parse($meter->installation_date)->format('M d, Y') }}</p>
                </div>
                <div>
            ...
            <div class="bg-white rounded-lg shadow-md p-6">
                ...
                <form action="{{ route('meter.readings.store', $meter) }}" method="POST" id="addReadingForm">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="number" name="reading_value" id="reading_value" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50" value="{{ old('reading_value') }}" required>
                        @error('reading_value')
                    ...
                    <div class="mb-6">
                        ...
                        <input type="date" name="reading_date" id="reading_date" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50" value="{{ old('reading_date') }}" required>
                        @error('reading_date')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
            ...
            <div class="bg-white rounded-lg shadow-md p-6">
                ...
                <form action="{{ route('meter.readings.estimate', $meter) }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="date" name="estimate_date" id="estimate_date" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50" value="{{ old('estimate_date') }}" required>
                        @error('estimate_date')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
    ...
    document.getElementById('addReadingForm').addEventListener('submit', function(event) {
        ...
        if (previousReading) {
            ...
            if (readingValue < minAcceptable || readingValue > maxAcceptable) {
                ...
                alert('The reading value is outside the acceptable range.');
            }
        }
    });
</script>
